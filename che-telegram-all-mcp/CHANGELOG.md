...
- **(#33) Multi-key handlers report the first bad field individually.** Combined messages like `"chat_id and text are required"` are replaced by per-field messages (`"chat_id is required"` / `"chat_id must be an integer; got ..."` / `"text is required"`). The combined form was itself misleading when only one field was bad (the #22 bug shape) — this is a net improvement, not just a change.
